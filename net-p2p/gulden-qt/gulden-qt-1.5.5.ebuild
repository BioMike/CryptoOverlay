...
DESCRIPTION="P2P Internet currency Gulden"
HOMEPAGE="https://www.gulden.com/"
SRC_URI="https://github.com/Gulden/${MyPN}/archive/v${MyPV}.tar.gz -> ${MyP}.tar.gz"
...
src_prepare() {
	epatch "${FILESDIR}/1.5.2_sys_leveldb.patch"
	eautoreconf
	rm -r src/leveldb
...
src_configure() {
	...
	# FIXME: add --with-system-libsecp256k1  \
}
...
src_install() {
	...
	newins "share/pixmaps/Gulden.ico" "${CapitalPN}.ico"

	make_desktop_entry "${CapitalPN} %u" "Gulden-Qt" "/usr/share/pixmaps/${CapitalPN}.ico" "Qt;Network;P2P;Office;Finance;" "MimeType=x-scheme-handler/gulden;\nTerminal=false"

	newman contrib/debian/manpages/bitcoin-qt.1 ${PN}.1
	newman contrib/debian/manpages/bitcoin.conf.5 ${PN}.conf.5
